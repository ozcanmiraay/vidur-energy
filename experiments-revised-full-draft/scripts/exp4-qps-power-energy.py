...
base_output_dir = (
    "/Users/mirayozcan/Desktop/vidur_copy/vidur/simulator_output/exp4-qps-power-energy"
)
...
qps_values_below_1 = np.round(
    np.logspace(np.log10(0.1), np.log10(1), num=num_points_per_range), 2
).tolist()
qps_values_above_1 = np.round(
    np.logspace(np.log10(2), np.log10(50), num=num_points_per_range), 1
).tolist()
...
for qps in qps_values:
    ...
    sim_output_dir = os.path.join(
        base_output_dir, f"{model_name.replace('/', '_')}_qps_{qps}"
    )
