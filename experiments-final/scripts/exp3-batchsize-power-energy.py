...
for max_batch_size in max_batch_sizes:
    ...
    sim_output_dir = os.path.join(
        base_output_dir, f"{model_name.replace('/', '_')}_batch_{max_batch_size}"
    )
    ...
    try:
        ...
        if not actual_sim_dir:
            print(
                f"No valid output found for {model_name}, Max Batch Size={max_batch_size}. Skipping..."
            )
            continue
        ...
        print(
            f"Energy analysis complete for {model_name}, Max Batch Size={max_batch_size}."
        )
            ...
            experiment_results.append(
                {
                    ...
                    "num_requests": num_requests,
                    "max_batch_size": max_batch_size,
                    "actual_batch_size_weighted_mean": stats.get(
                        "actual_batch_size_weighted_mean", None
                    ),
        ...
        print(
            f"Simulation failed for {model_name}, Max Batch Size={max_batch_size}: {e}"
        )
    except Exception as e:
        print(
            f"Error processing results for {model_name}, Max Batch Size={max_batch_size}: {e}"
        )
