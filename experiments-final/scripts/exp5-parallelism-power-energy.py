...
def main():
    args = parse_args()
    base_output_dir = os.path.join(args.output_dir, "exp5-parallelism-power-energy")
    os.makedirs(base_output_dir, exist_ok=True)

    # Fixed model for all tests
    model_name = "codellama/CodeLlama-34b-Instruct-hf"
    model_params = {"codellama/CodeLlama-34b-Instruct-hf": 34e9}  # Model parameter dictionary
    ...
    for setting in parallelization_settings:
        ...
        sim_output_dir = os.path.join(base_output_dir, f"{model_name.replace('/', '_')}_TP_{TP}_PP_{PP}")
