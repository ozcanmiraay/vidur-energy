...
def run_simulation(qps: float, output_dir: str):
    ...
    cmd = [
        ...
        "poisson",
        f"--poisson_request_interval_generator_config_qps",
        str(qps),
        # Number of requests
...
def main():
    ...
    qps_values = np.linspace(0.1, 1, 20)  # 20 evenly spaced points from 0.01 to 1
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    base_dir = os.path.join("simulator_output", f"qps_mini_study_{timestamp}")
    ...
    for qps in qps_values:
        ...
        sim_dir = os.path.join(
            base_dir, f"qps_{qps:.3f}"
        )  # Using 3 decimal places for mini values
