...
class EnergyVisualizer:
    def create_time_series_plot(self, df: pd.DataFrame) -> go.Figure:
        ...
        fig.update_layout(
            ...
            xaxis=dict(
                title=dict(text="Time (seconds)", font=dict(color=COLORS["text"])),
                gridcolor=COLORS["green_palette"][0],
                zeroline=False,
            ),
            yaxis=dict(
                title=dict(
                    text="Energy Consumption (kWh)", font=dict(color=COLORS["text"])
                ),
                gridcolor=COLORS["green_palette"][0],
                zeroline=False,
            ),
            yaxis2=dict(
                title=dict(
                    text="Model FLOPs Utilization (%)", font=dict(color=COLORS["text"])
                ),
                overlaying="y",
                side="right",
                gridcolor=COLORS["green_palette"][0],
    ...
    def create_efficiency_plot(self, df: pd.DataFrame) -> go.Figure:
        ...
        fig.add_trace(
            go.Scatter(
                x=df_downsampled[
                    "mfu"
                ].tolist(),  # Removed the *100 multiplication here too
                ...
                marker=dict(
                    ...
                    colorscale=[
                        [i / 4, color]
                        for i, color in enumerate(COLORS["green_palette"])
                    ],
                    showscale=True,
                    colorbar=dict(
                        title=dict(
                            text="Effective Power (W)", font=dict(color=COLORS["text"])
                        ),
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                title=dict(
                    text="Energy Efficiency (FLOPS/W)", font=dict(color=COLORS["text"])
                ),
                gridcolor=COLORS["green_palette"][0],
                zeroline=False,
            ),
    ...
    def create_regional_impact_plot(
        ...
        fig.update_layout(
            title=dict(
                text="Regional Impact Comparison", font=dict(color=COLORS["text"])
            ),
            xaxis=dict(
                title=dict(text="Impact Metrics", font=dict(color=COLORS["text"]))
            ),
            yaxis=dict(title=dict(text="Region", font=dict(color=COLORS["text"]))),
            barmode="group",
            plot_bgcolor="white",
            paper_bgcolor="white",
            showlegend=True,
