...
class SimulationEnergyAnalyzer:
    ...
    def get_configs(self) -> Tuple[str, int]:
        ...
        num_gpus = (config["cluster_config"]["num_replicas"] * 
                    config["cluster_config"]["replica_config"]["num_pipeline_stages"])
        
        if gpu_type not in GPU_POWER_CONFIGS:
            raise ValueError(f"Unsupported GPU type: {gpu_type}. Supported types: {list(GPU_POWER_CONFIGS.keys())}")
    ...
    def analyze_energy_consumption(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for region_name, region_cfg in REGIONAL_ENERGY_CONFIGS.items():
                ...
                total_metrics = {'energy_kwh': 0, 'energy_cost': 0, 'carbon_emissions': 0}
                
                for _, row in mfu_df.iterrows():
                    metrics = temp_calculator.calculate_energy(
                        gpu_hours=row['execution_time'] / 3600,
                        mfu=row['mfu']
                    )
            ...
            plots = {
                'energy_time_series': self.visualizer.create_time_series_plot(energy_df),
                'efficiency_scatter': self.visualizer.create_efficiency_plot(energy_df),
                'regional_impact': self.visualizer.create_regional_impact_plot(regional_metrics)
            }
            
            # Calculate total metrics
            total_metrics = {
                'energy_kwh': energy_df['energy'].sum(),
                'energy_cost': energy_df['energy'].sum() * region_config.electricity_cost,
                'carbon_emissions': (
