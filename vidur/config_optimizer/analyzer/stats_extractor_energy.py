...
def interpolate_power(mfu: float, gpu_type: str) -> float:
    ...
    utilization_factor = min(
        1.0, (mfu / TYPICAL_HIGH_MFU) ** 0.7
    )  # Power of 0.7 makes scaling more aggressive

    # Calculate power, capped at max_util
    power = (
        gpu_config.idle + (gpu_config.max_util - gpu_config.idle) * utilization_factor
    )
    return power
    ...
    for mfu_file in mfu_files:
        ...
        for i in range(1, len(df)):
            ...
            energy_consumption = calculate_energy_consumption(
                gpu_hrs, power_values, mfu, gpu_type
            )
...
def extract_utilization_stats(run_dir: str, stat_name: str):
    ...
    return {
        ...
        f"{stat_name}_peak": (
            max(max_vals) if max_vals else np.nan
...
def process_run(run_dir: str):
    ...
    try:
        prefill_tokens_per_request = config["request_generator_config"][
            "length_generator_config"
        ]["prefill_tokens"]
        decode_tokens_per_request = config["request_generator_config"][
            "length_generator_config"
        ]["decode_tokens"]
        total_tokens_processed = prefill_tokens_per_request + decode_tokens_per_request
    except KeyError:
        logger.warning(
            "Prefill and decode token counts not found in config. Setting to default values."
        )
    ...
    def extract_batch_size_stats(run_dir):
        ...
        for file in batch_size_files:
            ...
            for key, value in data.items():
                if key.endswith("_mfu_distribution") and isinstance(value, list):
                    ...
                    batch_sizes = [
                        entry["batch_size"] for entry in value if "batch_size" in entry
                    ]

                    if batch_sizes:
                        all_batch_sizes.extend(batch_sizes)
                        batch_stage_counts.extend(
                            [1] * len(batch_sizes)
                        )  # Assign weight of 1 per batch size

        if not all_batch_sizes:
            print("⚠️ No batch sizes found in any files!")
            return {
                "actual_batch_size_weighted_mean": np.nan,
                "actual_batch_size_std": np.nan,
            }
        ...
        if batch_sizes_array.shape != batch_stage_counts_array.shape:
            print(
                f"Shape mismatch detected: batch_sizes {batch_sizes_array.shape}, batch_stage_counts {batch_stage_counts_array.shape}"
            )
            batch_stage_counts_array = np.ones_like(
                batch_sizes_array
            )  # Assign equal weights if mismatch occurs

        # Compute weighted mean batch size
        weighted_mean_batch_size = np.average(
            batch_sizes_array, weights=batch_stage_counts_array
        )
...
def process_trace(sim_results_dir: str, region: str = "california"):
    ...
    prefill_tokens_per_request = (
        df["prefill_tokens_per_request"].iloc[0]
        if "prefill_tokens_per_request" in df.columns
        else np.nan
    )
    decode_tokens_per_request = (
        df["decode_tokens_per_request"].iloc[0]
        if "decode_tokens_per_request" in df.columns
        else np.nan
    )
    total_tokens_processed = (
        df["total_tokens_processed"].sum()
        if "total_tokens_processed" in df.columns
        else np.nan
    )
    ...
    if "poisson_request_interval_generator_qps" in df.columns:
        ...
        df["capacity_per_replica"] = df["poisson_request_interval_generator_qps"] / df[
            "cluster_config"
        ].apply(lambda x: x["num_replicas"])
    else:
    ...
    df["num_replica_gpus"] = df["cluster_config"].apply(
        lambda x: x["replica_config"]["tensor_parallel_size"]
    ...
    simulation_stats.update(
        {
            ...
            "prefill_tokens_per_request": prefill_tokens_per_request,
            "decode_tokens_per_request": decode_tokens_per_request,
            "total_tokens_processed": num_requests
            * total_tokens_processed,  # Sum over all requests
        }
    )

    simulation_stats.update(
        {
            "actual_batch_size_weighted_mean": df[
                "actual_batch_size_weighted_mean"
            ].mean(),
    ...
    simulation_stats.update(
        {
            ...
            "e2e_latency_95p": df[
                "e2e_latency_95%"
            ].mean(),  # 95th percentile end-to-end latency
            "decode_latency_95p": df[
                "decode_latency_95%"
            ].mean(),  # 95th percentile decode latency
        }
    )

    simulation_stats["tokens_per_second"] = (
        (simulation_stats["total_tokens_processed"] / sim_time)
        if sim_time > 0
        else np.nan
    )
